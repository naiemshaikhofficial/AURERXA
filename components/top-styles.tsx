...
export async function TopStyles() {
    ...
    const [all, earrings, chains, rings, bracelets] = await Promise.all([
        getFilteredProducts({ sortBy: 'popular' }),
        getFilteredProducts({ category: 'earrings', sub_category: 'earrings', tag: 'earrings', sortBy: 'popular' }),
        ...
        getFilteredProducts({ sub_category: 'bracelets', tag: 'bracelets', sortBy: 'popular' })
    ])

    const dataMap = {
        all: all ? all.slice(0, 8) : [],
        earrings: earrings ? earrings.slice(0, 8) : [],
