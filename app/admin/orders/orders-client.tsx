...
export function OrdersClient({ initialOrders, total, adminRole }: { initialOrders: any[], total: number, adminRole: string | null }) {
    ...
    const handleBulkStatusUpdate = async (newStatus: string) => {
        ...
        if (!confirm(`Update ${selectedOrderIds.size} orders to ${newStatus.toUpperCase()}?`)) return

        setIsBulkUpdating(true)
        const res = await bulkUpdateOrderStatus(Array.from(selectedOrderIds), newStatus)
    ...
    const handleBulkPrint = (type: 'customer' | 'shipping') => {
        setPrintType(type)
        setTimeout(() => {
            window.print()
            setPrintType(null)
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-sm text-white/40 h-11">
                    ...
                    <span className="text-[10px] uppercase font-bold tracking-widest hidden lg:inline">From:</span>
                    <input
                        type="date"
                ...
                <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-sm text-white/40 h-11">
                    <Clock className="w-4 h-4" />
                    <span className="text-[10px] uppercase font-bold tracking-widest hidden lg:inline">To:</span>
                    <input
                        type="date"
                ...
                <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-sm text-white/40 h-11">
                    ...
                    <span className="text-[10px] uppercase font-bold tracking-widest hidden lg:inline text-nowrap">Total:</span>
                    <input
                        type="number"
                        placeholder="Min"
                        defaultValue={currentMinTotal}
                        onBlur={e => updateFilters('minTotal', e.target.value)}
                    ...
                    <input
                        type="number"
                        placeholder="Max"
                        defaultValue={currentMaxTotal}
                        onBlur={e => updateFilters('maxTotal', e.target.value)}
                        className="w-full bg-transparent text-white focus:outline-none placeholder:text-white/20"
                    />
                </div>
                {(currentDateFrom || currentDateTo || currentMinTotal || currentMaxTotal || currentSearch !== '' || currentStatus !== 'all') && (
                    <button
                        onClick={() => {
                            router.push(pathname)
                        }}
                        className="flex items-center justify-center gap-2 px-4 py-2 bg-red-500/10 text-red-400 rounded-xl text-xs hover:bg-red-500/20 transition h-11"
                    >
            ...
            {selectedOrderIds.size > 0 && (
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-[#111111] border border-[#D4AF37]/30 rounded-2xl p-4 shadow-2xl shadow-[#D4AF37]/10 flex items-center gap-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                    <div className="flex items-center gap-3 pr-6 border-r border-white/10">
                        <div className="w-8 h-8 rounded-lg bg-[#D4AF37] text-black flex items-center justify-center font-bold text-sm">
                            {selectedOrderIds.size}
                        </div>
                        <p className="text-white font-medium text-sm whitespace-nowrap">Orders Selected</p>
                    </div>

                    <div className="flex flex-wrap items-center gap-2">
                        <button
                            onClick={() => handleBulkPrint('customer')}
                            className="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-xl text-xs font-semibold uppercase tracking-wider transition-all flex items-center gap-2"
                        >
                            <FileText className="w-4 h-4" /> Print Invoices
                        </button>
                        <button
                            onClick={() => handleBulkPrint('shipping')}
                            className="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-xl text-xs font-semibold uppercase tracking-wider transition-all flex items-center gap-2"
                        >
                            <Truck className="w-4 h-4" /> Print Labels
                        </button>

                        <div className="w-px h-6 bg-white/10 mx-2" />

                        <div className="flex items-center gap-2">
                            <span className="text-[10px] text-white/30 uppercase font-bold tracking-widest whitespace-nowrap">Status:</span>
                            <div className="flex gap-1">
                                {['packed', 'shipped', 'delivered'].map(status => (
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${isBulkUpdating ? 'opacity-50 cursor-wait' : 'hover:scale-105 active:scale-95'
                                ...
                                <tr className="border-b border-white/5">
                                    <th className="w-10 px-4 py-3">
                                        <input
                                            ...
                                            className="rounded border-white/10 bg-white/5 text-[#D4AF37] focus:ring-offset-0 focus:ring-0"
                                        />
                                    </th>
                                    ...
                                    return (
                                        <tr
                                            ...
                                            className={cn(
                                                ...
                                                selectedOrderIds.has(order.id) && "bg-[#D4AF37]/5"
                                            )}
                        ...
                        {orders.map(order => {
                            ...
                            return (
                                <div
                                    ...
                                    onClick={() => setSelectedOrder(order)}
                                    className={cn(
                                        "bg-[#111111] border rounded-xl p-4 active:bg-white/[0.02] transition relative",
                                        isHighlighted ? "border-[#D4AF37] ring-1 ring-[#D4AF37] shadow-lg shadow-[#D4AF37]/10" : "border-white/5",
                                        selectedOrderIds.has(order.id) && "border-[#D4AF37]/30 bg-[#D4AF37]/5"
                                    )}
                                >
                                    <div className="absolute top-4 right-4" onClick={e => e.stopPropagation()}>
                                        <input
                                            ...
                                            checked={selectedOrderIds.has(order.id)}
                                            onChange={e => toggleSelectOrder(order.id, e as any)}
                                            className="rounded border-white/20 bg-white/10 text-[#D4AF37] focus:ring-offset-0 focus:ring-0"
                                        />
                                    </div>
            ...
            {printType && typeof document !== 'undefined' && createPortal(
                <div id="print-root" className="fixed left-[-9999px] top-0 z-[99999] pointer-events-none overflow-hidden print:left-0 print:static print:w-full print:opacity-100">
                    <div className="flex flex-col items-center justify-start">
                        {/* If single selection from drawer */}
                        {!selectedOrderIds.size && selectedOrder && (
                            <div className={cn(
                                "bg-white",
                                printType === 'shipping' ? "w-[100mm]" : "w-full max-w-[800px]"
                            )}>
                                <InvoiceTemplate order={selectedOrder as any} type={printType as any} />
                            </div>
                        )}

                        {/* Bulk selection */}
                        {selectedOrderIds.size > 0 && Array.from(selectedOrderIds).map(id => {
                            const order = orders.find(o => o.id === id)
                            if (!order) return null
                            return (
                                <div key={id} className={cn(
                                    "bg-white print-break",
                                    printType === 'shipping' ? "w-[100mm]" : "w-full max-w-[800px]"
                                )}>
                                    <InvoiceTemplate order={order as any} type={printType as any} />
                                </div>
                            )
                        })}
                    </div>
