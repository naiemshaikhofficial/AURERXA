...
export function DashboardClient({
    ...
    const [revenueData, setRevenueData] = useState<any[]>(initialRevenue)
    const [ordersData, setOrdersData] = useState<any[]>(initialOrders)
    const [recentOrders, setRecentOrders] = useState<any[]>(initialRecent)
    const [topProducts, setTopProducts] = useState<any[]>(initialTopProducts)
    const [activityLogs, setActivityLogs] = useState<any[]>(initialActivity)
    const [cancelledDetails, setCancelledDetails] = useState<any[]>(initialCancelled)
    const [adminRole, setAdminRole] = useState<string>(initialAdminRole)
    ...
    // Only reload when date filters change, NOT on mount (data is passed via props)
    useEffect(() => {
        // Skip initial load if using default preset which matches server-side fetch
        if (datePreset === 'month' && !customFrom && !loading) return
        loadData()
    }, [datePreset, customFrom, customTo])

    // Reload charts when period changes
    useEffect(() => {
        // Avoid reloading if it matches initial state, but for simplicity we can reload.
        // Actually optimization: if revenuePeriod matches what we fetched server-side?
        // Server side fetched 'monthly'.
