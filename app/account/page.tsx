        ...
        <div className="min-h-screen bg-neutral-950 text-white">
            ...
            <main className="pt-16 md:pt-24 pb-24">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <p className="text-white/50 text-center mb-12">{profile?.email}</p>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-12">
                        <Link href="/account/orders" className="bg-neutral-900 border border-neutral-800 p-6 text-center hover:border-amber-500/30 transition-all">
                            <Package className="w-8 h-8 mx-auto mb-3 text-amber-500" />
                            <p className="font-medium">My Orders</p>
                        </Link>
                        <Link href="/wishlist" className="bg-neutral-900 border border-neutral-800 p-6 text-center hover:border-amber-500/30 transition-all">
                            <Star className="w-8 h-8 mx-auto mb-3 text-amber-500" />
                            <p className="font-medium">Wishlist</p>
                        </Link>
                        <button onClick={handleSignOut} className="bg-neutral-900 border border-neutral-800 p-6 text-center hover:border-red-500/30 transition-all">
                    ...
                    <div className="bg-neutral-900 border border-neutral-800 p-6 mb-8">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            {!editingProfile && (
                                <button onClick={() => setEditingProfile(true)} className="text-sm text-amber-500 hover:text-amber-400 flex items-center gap-1">
                        ...
                        {editingProfile ? (
                            <form onSubmit={handleUpdateProfile} className="space-y-4">
                                ...
                                <div className="flex gap-2">
                                    <Button type="submit" disabled={saving} size="sm" className="bg-amber-500 hover:bg-amber-400 text-neutral-950">
                                        {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Save'}
                                    </Button>
                                    <Button type="button" onClick={() => setEditingProfile(false)} variant="outline" size="sm" className="border-neutral-700">
                                        Cancel
                                    </Button>
                                </div>
                            </form>
                            ...
                            <div className="space-y-2 text-sm">
                                <p><span className="text-white/50">Name:</span> {profile?.full_name || 'Not set'}</p>
                                <p><span className="text-white/50">Email:</span> {profile?.email}</p>
                                <p><span className="text-white/50">Phone:</span> {profile?.phone_number || 'Not set'}</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-neutral-900 border border-neutral-800 p-6">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            {addresses.length < 5 && !showAddressForm && !editingAddressId && (
                                <button onClick={() => setShowAddressForm(true)} className="text-sm text-amber-500 hover:text-amber-400 flex items-center gap-1">
                        ...
                        {(showAddressForm || editingAddressId) && (
                            <form onSubmit={editingAddressId ? handleUpdateAddress : handleAddAddress} className="mb-6 p-4 border border-neutral-700 space-y-4">
                        ...
                        {addresses.length === 0 && !showAddressForm ? (
                            <p className="text-white/50 text-center py-8">No addresses saved yet.</p>
                        ) : (
                            <div className="space-y-3">
                                {addresses.map((addr) => (
                                    <div key={addr.id} className="p-4 border border-neutral-700 hover:border-neutral-600 transition-all">
                                        <div className="flex items-start justify-between">
                                            ...
                                            <div className="flex items-center gap-2">
                                                {!addr.is_default && (
                                                    <button onClick={() => handleSetDefault(addr.id)} className="text-xs text-amber-500 hover:text-amber-400">
                                                        Set Default
                                                    </button>
                                                )}
                                                <button onClick={() => startEditAddress(addr)} className="p-2 text-white/50 hover:text-amber-400">
                                                    <Edit2 className="w-4 h-4" />
                                                </button>
                                                <button onClick={() => handleDeleteAddress(addr.id)} className="p-2 text-white/50 hover:text-red-400">
